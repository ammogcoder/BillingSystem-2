...
Public Class CFDx
    ...
    ''' <remarks></remarks>
    Public Function CreaFacturaXML(ByVal KeyFile As String, ByVal Pass As String, ByVal CerFile As String, ByRef Errores As String) As Boolean
        Dim Resultado As Boolean = False
        Dim c As Comprobante = CreaXML()

        Dim Path As String = Environment.CurrentDirectory
        Dim XMLfile As String = IO.Path.Combine(Path, "CFDI.xml")
        ...
        SellarXML(KeyFile, Pass, CerFile, xDoc)
...
#Region "Sello Digital"
    ...
    ''' <remarks></remarks>
    Private Sub SellarXML(ByVal KeyFile As String, ByVal Pass As String, _
                            ByVal CerFile As String, ByRef XMLstring As XDocument)
        ...
        '  Load the private key from an RSA .key f0ile:
        If privKey.LoadPkcs8EncryptedFile(KeyFile, Pass) = False Then
        ...
        If rsa.UnlockComponent("RSA!TEAM!BEAN_3351D4535RlU") = False Then
        ...
        If rsa2.VerifyStringENC(Cadena, Hash, Sellobase64) = False Then
            'MsgBox(rsa2.LastErrorText)
            Console.Write(rsa2.LastErrorText)
            Console.Read()
            Exit Sub
        End If
    ...
    End Sub
    Private Sub AgregaCertificado(ByVal CerFile As String, ByRef XMLstring As XDocument)
        Dim cert As New Chilkat.Cert
        ''  Load a digital certificate from a .cer file:
        If cert.LoadFromFile(CerFile) = False Then
            MsgBox(cert.LastErrorText)
    ...
    Public Sub AbreCFDIcredo(ByVal XMLfile As String)
